...
class SmsOperator
{
    ...
    {
        $this->yunpian_config = C('YUNPIAN_CONFIG');
        if ($api_secret == null)
    ...
    public function single_send($data = array())
    {
        ...
        return \Org\Util\sms\HttpUtil::PostCURL($this->yunpian_config['URI_SEND_SINGLE_SMS'], $data);
    }
    ...
    public function batch_send($data = array())
    {
        ...
        return \Org\Util\sms\HttpUtil::PostCURL($this->yunpian_config['URI_SEND_BATCH_SMS'], $data);
    }
    ...
    public function multi_send($data = array())
    {
        ...
        return \Org\Util\sms\HttpUtil::PostCURL($this->yunpian_config['URI_SEND_MULTI_SMS'], $data);
    }

    public function tpl_send($data = array())
    {
        if (!array_key_exists('mobile', $data))
            return new \Org\Util\sms\Result(null, $data, null, 'mobile 为空');
        if (!array_key_exists('tpl_id', $data))
            return new \Org\Util\sms\Result(null, $data, null, 'tpl_id 为空');
        if (!array_key_exists('tpl_value', $data))
            return new \Org\Util\sms\Result(null, $data, null, 'tpl_value 为空');

        $data['apikey'] = $this->apikey;

        return \Org\Util\sms\HttpUtil::PostCURL($this->yunpian_config['URI_SEND_TPL_SMS'], $data);
    }
}
